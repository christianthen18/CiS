...
@section('content')
    <div class="page-content">
        <div class="row">
            <div class="col-md-12">
                <h2 class="text-center mb-4">Purchase Return Information</h2>
                ...
                <div class="card mb-4">
                    ...
                    <div id="searchCollapse" class="collapse {{ request()->hasAny(['search', 'status', 'date_from', 'date_to']) ? 'show' : '' }}">
                        <div class="card-body">
                            <form action="{{ route('purchaseRetur.index') }}" method="GET">
                                <div class="row">
                                    <!-- Invoice/Product Search -->
                                    <div class="col-md-6 mb-3">
                                        <label for="search">Search by invoice or product</label>
                                        <input type="text" id="search" name="search" class="form-control" placeholder="Enter invoice number or product name" value="{{ request('search') }}">
                                    </div>
                                ...
                                <div class="text-right">
                                    <a href="{{ route('purchaseRetur.index') }}" class="btn btn-secondary">Reset</a>
                ...
                @if(request()->hasAny(['search', 'status', 'date_from', 'date_to']))
                    <div class="alert alert-info">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('purchaseRetur.index') }}" class="btn btn-sm btn-outline-secondary">Clear All Filters</a>
                        </div>
                    </div>
                @endif
                
                <div class="d-flex justify-content-end mb-3">
                    <a href="{{ route('purchaseRetur.create') }}" class="btn btn-info"
                        ...
                        @foreach ($returs as $retur)
                            <div class="col-md-4 mb-4">
                                <div class="card shadow-sm h-100">
                                    <!-- Return Details -->
                                    <div class="card-body">
                                        <h3 class="card-title text-center">Purchase Return</h3>
                                        ...
                                        <p class="card-text text-center">
                                            <strong>Product:</strong> {{ $retur->product->name ?? 'N/A' }}
                                        </p>
                                        <div class="text-center">
                                            <a class="btn btn-warning btn-sm"
                                                style="background-color: rgb(9, 9, 9); color: white; margin-right: 5px;"
                                                href="{{ route('purchaseRetur.detail', $retur->id) }}">Detail</a>
                                            <form method="POST" action="{{ route('purchaseRetur.destroy', $retur->id) }}"
                    ...
                    @else
                        <div class="col-12">
                            <div class="alert alert-warning text-center">
                                <h4><i class="fas fa-exclamation-triangle"></i> No purchase returns found</h4>
                                <p>Try adjusting your search criteria or create a new return</p>
                                <div>
                                    <a href="{{ route('purchaseRetur.index') }}" class="btn btn-outline-primary mr-2">Show All Returns</a>
                                    <a href="{{ route('purchaseRetur.create') }}" class="btn btn-primary">Create New Return</a>
                                </div>
                            </div>
                        </div>
                    @endif
